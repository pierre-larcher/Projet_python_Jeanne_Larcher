...
def _ordre(depart:str,arrivee:str)->str:
    """
Fonctionnalité:
...
def _temps_depuis_depart(graphe: Graphe, depart: str, arrivee: str) -> tuple[dict,float]:
    """
...
def _recuperation_candidat_min_chemin(graphe: Graphe, depart: str, arrivee : str) -> list[str, float]:
    """
...
"""
    candidat = list()
    temps = _temps_depuis_depart(graphe,depart,arrivee)
...
def _separate(graphe:Graphe, depart : str, arrivee : str)->list():
    """
...
"""
    temps = _temps_depuis_depart(graphe,depart,arrivee)
    liste = _recuperation_candidat_min_chemin(graphe,depart,temps)
    sens = _ordre(depart,arrivee)
    liste1 = list()
    liste_inter = list()
    liste_final = list()
    nb = 0
...
def _trouve_chemin_minimal(graphe:Graphe, depart : str, arrivee : str)->list():
    """
    ...
    -------------------------------
    """
    listes = _separate(graphe,depart,arrivee)
    newlist = list()
    liste_fi = list()
    for liste in listes:
...
def _organisation_ordre(graphe:Graphe, depart : str, arrivee : str)->list():
    """
...
-------------------------------
"""
    all = _trouve_chemin_minimal(graphe, depart, arrivee)
...
def Le_plus_cours_chemin(graphe:Graphe, depart : str, arrivee : str):
    """
...
-------------------------------
"""
    chemin = _organisation_ordre(graphe,depart,arrivee)
    resultat = _calcule_temps(graphe,depart,arrivee)
    for i in chemin:
        print(f"Dirigez-vous de {i[1]} vers {i[0]}, le temps pour s'y rendre sera de {i[3]} minutes")
    print(f"Le temps total pour se rendre de {depart} à {arrivee} est de {resultat} minutes")